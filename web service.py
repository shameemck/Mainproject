...
def viewreply():
    ...
    print(login_id)
    cmd.execute("SELECT `complaint`.*,`department`.* FROM `complaint` JOIN `department` ON `department`.`dpid`=`complaint`.`dp id` WHERE `complaint`.`lid`='"+str(login_id)+"' AND `complaint`.`reply`!='pending'")
